    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            If Request.QueryString("Save") IsNot Nothing AndAlso Request.QueryString("Save") = "OK" Then
                Bootstrap_Panel.ShowMessage("گروه دسترسی با موفقیت ذخیره شد", False)
            ElseIf Request.QueryString("Edit") IsNot Nothing AndAlso Request.QueryString("Edit") = "OK" Then
                Bootstrap_Panel.ShowMessage("گروه دسترسی با موفقیت ویرایش شد", False)
            ElseIf Request.QueryString("Save") IsNot Nothing AndAlso Request.QueryString("Save") = "NO" Then
                Bootstrap_Panel.ShowMessage("در فرآیند ذخیره گروه دسترسی خطا رخ داده است", True)
            ElseIf Request.QueryString("Edit") IsNot Nothing AndAlso Request.QueryString("Edit") = "NO" Then
                Bootstrap_Panel.ShowMessage("در فرآیند ویرایش گروه دسترسی خطا رخ داده است", True)
            End If
    ...
    Private Sub Bootstrap_Panel_Panel_Delete_Click(sender As Object, e As EventArgs) Handles Bootstrap_Panel.Panel_Delete_Click
        ...
        Try
            For i As Integer = 0 To arrstrTheKeys.Length - 2
                ...
                qryAccessgroup.spr_Accessgroup_Delete(inttheKey)
            Next i
...
#Region "Table Functions"
    ...
    Private Function GetRecordCount() As Integer
        ...
        Dim intRecordCount = OdbVAS.tbl_Accessgroup.Count
    ...
    Private Function GetRecordCount(strFilterText As String) As Integer
        ...
        Dim intRecordCount = OdbVAS.tbl_Accessgroup.Where(Function(x) x.Desp.IndexOf(strFilterText) <> -1).Count
        Return intRecordCount
    ...
    Private Sub FillTable()
        ...
        If intCurrentPageNo > 0 Then
            ...
            Dim OdbVAS As New BusinessObject.Context.dbVASEntities
            Dim lnqAccessgroup = OdbVAS.tbl_Accessgroup.OrderByDescending(Function(x) x.ID).Skip(intSkip).Take(mdlGeneral.cnst_RowsCountInPage).ToList()
            intStartRecord = (intCurrentPageNo - 1) * mdlGeneral.cnst_RowsCountInPage + 1
            intEndRecord = intStartRecord + lnqAccessgroup.Count - 1
            ...
            For Each lnqAccessgroupItem In lnqAccessgroup
                ...
                tbCell.InnerHtml = "<input type = ""checkbox"" value="""" id=""chkk" & lnqAccessgroupItem.ID & """   />"
                ...
                tbCell = New HtmlTableCell
                tbCell.InnerHtml = "<a href=""#"" onclick=""btnEdit_ClientClick(" & lnqAccessgroupItem.ID & ")"">&nbsp;" & lnqAccessgroupItem.Desp & "&nbsp;</a>"
    ...
    Private Sub FillTable(strFilterText As String)
        ...
        If intCurrentPageNo > 0 Then
            ...
            Dim lnqAccessgroup = OdbVAS.tbl_Accessgroup.Where(Function(x) x.Desp.IndexOf(strFilterText) <> -1).OrderByDescending(Function(x) x.ID).Skip(intSkip).Take(mdlGeneral.cnst_RowsCountInPage).ToList()
