    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            If Request.QueryString("Save") IsNot Nothing AndAlso Request.QueryString("Save") = "OK" Then
                Bootstrap_Panel.ShowMessage("کاربر با موفقیت ذخیره شد", False)
            ElseIf Request.QueryString("Edit") IsNot Nothing AndAlso Request.QueryString("Edit") = "OK" Then
                Bootstrap_Panel.ShowMessage("کاربر با موفقیت ویرایش شد", False)
            ElseIf Request.QueryString("NoAccess") IsNot Nothing AndAlso Request.QueryString("NoAccess") = "Yes" Then
                Bootstrap_Panel.ShowMessage("شما دسترسی کافی برای اصلاح این کاربر را ندارید", True)
            ElseIf Request.QueryString("Save") IsNot Nothing AndAlso Request.QueryString("Save") = "NO" Then
                Bootstrap_Panel.ShowMessage("در فرآیند ذخیره کاربر خطا رخ داده است", True)
            ElseIf Request.QueryString("Edit") IsNot Nothing AndAlso Request.QueryString("Edit") = "NO" Then
                Bootstrap_Panel.ShowMessage("در فرآیند ویرایش کاربر خطا رخ داده است", True)
            End If
    ...
    Private Sub Bootstrap_Panel_Panel_Delete_Click(sender As Object, e As EventArgs) Handles Bootstrap_Panel.Panel_Delete_Click
        ...
        Try
            For i As Integer = 0 To arrstrTheKeys.Length - 2
                ...
                qryUser.spr_User_Delete(inttheKey)
            Next i
...
#Region "Table Functions"
    ...
    Private Function GetRecordCount() As Integer
        ...
        Dim intRecordCount = OdbVAS.tbl_User.Count
    ...
    Private Function GetRecordCount(strFilterText As String) As Integer
        ...
        Dim OdbVAS As New BusinessObject.Context.dbVASEntities
        '   Dim intRecordCount = OdbVAS.tbl_User.Where(Function(x) x.Username.ToUpper.IndexOf(strFilterText.ToUpper) <> -1 OrElse x.NationalID.IndexOf(strFilterText) <> -1 OrElse x.IsReal = If(strFilterText = "حقیقی", True, False) OrElse (x.FName + " " + x.LName).IndexOf(strFilterText) <> -1 OrElse If(x.IsDataAdmin = True AndAlso x.IsItemAdmin = True, "FULL", If(x.IsDataAdmin = True AndAlso x.IsItemAdmin = False, "DATA", If(x.IsDataAdmin = False AndAlso x.IsItemAdmin = True, "ITEM", "NORMAL"))).IndexOf(strFilterText.ToUpper) <> -1).Count
        Dim intRecordCount = OdbVAS.tbl_User.Where(Function(x) x.Username.ToUpper.IndexOf(strFilterText.ToUpper) <> -1 OrElse x.NationalID.IndexOf(strFilterText) <> -1 OrElse If(x.IsReal = True, "حقیقی", "حقوقی").IndexOf(strFilterText) <> -1 OrElse (x.FName + " " + x.LName).IndexOf(strFilterText) <> -1 OrElse If(x.IsDataAdmin = True AndAlso x.IsItemAdmin = True, "FULL", If(x.IsDataAdmin = True AndAlso x.IsItemAdmin = False, "DATA", If(x.IsDataAdmin = False AndAlso x.IsItemAdmin = True, "ITEM", "NORMAL"))).IndexOf(strFilterText.ToUpper) <> -1).Count
    ...
    Private Sub FillTable()
        ...
        If intCurrentPageNo > 0 Then
            ...
            Dim OdbVAS As New BusinessObject.Context.dbVASEntities
            Dim lnqUser = OdbVAS.tbl_User.OrderByDescending(Function(x) x.ID).Skip(intSkip).Take(mdlGeneral.cnst_RowsCountInPage).ToList()
            intStartRecord = (intCurrentPageNo - 1) * mdlGeneral.cnst_RowsCountInPage + 1
            intEndRecord = intStartRecord + lnqUser.Count - 1

            Dim i As Integer = intStartRecord
            For Each lnqUserItem In lnqUser
                ...
                tbCell.InnerHtml = "<input type = ""checkbox"" value="""" id=""chkk" & lnqUserItem.ID & """   />"
                ...
                tbCell = New HtmlTableCell
                tbCell.InnerHtml = "<a href=""#"" onclick=""btnEdit_ClientClick(" & lnqUserItem.ID & ")"">&nbsp;" & lnqUserItem.Username & "&nbsp;</a>"
                ...
                tbCell = New HtmlTableCell
                tbCell.InnerHtml = lnqUserItem.FName & " " & lnqUserItem.LName
                tbRow.Cells.Add(tbCell)


                tbCell = New HtmlTableCell
                Dim strAccessType As String = "Normal"
                If lnqUserItem.IsItemAdmin = True AndAlso lnqUserItem.IsDataAdmin = True Then
                    strAccessType = "Full"
                ElseIf lnqUserItem.IsItemAdmin = True Then
                    strAccessType = "Item"
                ElseIf lnqUserItem.IsDataAdmin = True Then
                    strAccessType = "Data"
                End If
                tbCell.InnerHtml = strAccessType
                tbRow.Cells.Add(tbCell)

                tbCell = New HtmlTableCell
                tbCell.InnerHtml = lnqUserItem.NationalID
                tbRow.Cells.Add(tbCell)


                tbCell = New HtmlTableCell
                tbCell.InnerHtml = If(lnqUserItem.IsReal = True, "حقیقی", "حقوقی")
                tbRow.Cells.Add(tbCell)
    ...
    Private Sub FillTable(strFilterText As String)
        ' OdbVAS.tbl_User.Where(Function(x) x.Username.ToUpper.IndexOf(strFilterText.ToUpper) <> -1 OrElse x.NationalID.IndexOf(strFilterText) <> -1 OrElse x.IsReal = If(strFilterText = "حقیقی", True, False) OrElse (x.FName + " " + x.LName).IndexOf(strFilterText) <> 0 OrElse If(x.IsDataAdmin = True AndAlso x.IsItemAdmin = True, "FULL", If(x.IsDataAdmin = True AndAlso x.IsItemAdmin = False, "DATA", If(x.IsDataAdmin = False AndAlso x.IsItemAdmin = True, "ITEM", "NORMAL"))).IndexOf(strFilterText.ToUpper) <> -1)
        ...
        If intCurrentPageNo > 0 Then
            ...
            Dim lnqUser = OdbVAS.tbl_User.Where(Function(x) x.Username.ToUpper.IndexOf(strFilterText.ToUpper) <> -1 OrElse x.NationalID.IndexOf(strFilterText) <> -1 OrElse If(x.IsReal = True, "حقیقی", "حقوقی").IndexOf(strFilterText) <> -1 OrElse (x.FName + " " + x.LName).IndexOf(strFilterText) <> -1 OrElse If(x.IsDataAdmin = True AndAlso x.IsItemAdmin = True, "FULL", If(x.IsDataAdmin = True AndAlso x.IsItemAdmin = False, "DATA", If(x.IsDataAdmin = False AndAlso x.IsItemAdmin = True, "ITEM", "NORMAL"))).IndexOf(strFilterText.ToUpper) <> -1).OrderByDescending(Function(x) x.ID).Skip(intSkip).Take(mdlGeneral.cnst_RowsCountInPage).ToList()

            intStartRecord = (intCurrentPageNo - 1) * cnst_RowsCountInPage + 1
            intEndRecord = intStartRecord + lnqUser.Count - 1
