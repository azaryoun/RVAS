Public Class AggregatorsManagement
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            If Request.QueryString("Save") IsNot Nothing AndAlso Request.QueryString("Save") = "OK" Then
                Bootstrap_Panel.ShowMessage("تجمیع کننده با موفقیت ذخیره شد", False)
            ElseIf Request.QueryString("Edit") IsNot Nothing AndAlso Request.QueryString("Edit") = "OK" Then
                Bootstrap_Panel.ShowMessage("تجمیع کننده با موفقیت ویرایش شد", False)
            ElseIf Request.QueryString("Save") IsNot Nothing AndAlso Request.QueryString("Save") = "NO" Then
                Bootstrap_Panel.ShowMessage("در فرآیند ذخیره تجمیع کننده خطا رخ داده است", True)
            ElseIf Request.QueryString("Edit") IsNot Nothing AndAlso Request.QueryString("Edit") = "NO" Then
                Bootstrap_Panel.ShowMessage("در فرآیند ویرایش تجمیع کننده خطا رخ داده است", True)
            End If
        ...
        Else
            ...
            If hdnAction.Value.StartsWith("E") = True Then
                ...
                Response.Redirect("AggregatorsEdit.aspx")
            End If
...
#Region "Bootstrap_Panel Events"
    ...
    Private Sub Bootstrap_Panel_Panel_New_Click(sender As Object, e As EventArgs) Handles Bootstrap_Panel.Panel_New_Click
        Response.Redirect("AggregatorsNew.aspx")
    ...
    Private Sub Bootstrap_Panel_Panel_Delete_Click(sender As Object, e As EventArgs) Handles Bootstrap_Panel.Panel_Delete_Click
        ...
        Try
            For i As Integer = 0 To arrstrTheKeys.Length - 2
                ...
                qryAggregators.spr_Aggregators_Delete(inttheKey)
            Next i
...
#Region "Table Functions"
    ...
    Private Function GetRecordCount() As Integer
        ...
        Dim intRecordCount = OdbVAS.tbl_Aggregators.Count
    ...
    Private Function GetRecordCount(strFilterText As String) As Integer
        ...
        Dim intRecordCount = OdbVAS.tbl_Aggregators.Where(Function(x) x.Name.IndexOf(strFilterText) <> -1 OrElse x.IMIPreNumber.IndexOf(strFilterText) <> -1 OrElse x.PardisPreNumber.IndexOf(strFilterText) <> -1).Count
        Return intRecordCount
    ...
    Private Sub FillTable()
        ...
        If intCurrentPageNo > 0 Then
            ...
            Dim OdbVAS As New BusinessObject.Context.dbVASEntities
            Dim lnqAggergators = OdbVAS.tbl_Aggregators.OrderByDescending(Function(x) x.ID).Skip(intSkip).Take(mdlGeneral.cnst_RowsCountInPage).ToList()
            intStartRecord = (intCurrentPageNo - 1) * mdlGeneral.cnst_RowsCountInPage + 1
            intEndRecord = intStartRecord + lnqAggergators.Count - 1

            Dim i As Integer = intStartRecord
            For Each lnqAggergatorsItem In lnqAggergators
                ...
                tbCell.InnerHtml = "<input type = ""checkbox"" value="""" id=""chkk" & lnqAggergatorsItem.ID & """   />"
                ...
                tbCell = New HtmlTableCell
                tbCell.InnerHtml = "<a href=""#"" onclick=""btnEdit_ClientClick(" & lnqAggergatorsItem.ID & ")"">&nbsp;" & lnqAggergatorsItem.Name & "&nbsp;</a>"
                ...
                tbCell = New HtmlTableCell
                tbCell.InnerHtml = lnqAggergatorsItem.PardisPreNumber
                tbRow.Cells.Add(tbCell)

                tbCell = New HtmlTableCell
                tbCell.InnerHtml = lnqAggergatorsItem.IMIPreNumber
                tbRow.Cells.Add(tbCell)

                tblManagement.Rows.Insert(tblManagement.Rows.Count - 1, tbRow)
                i += 1
            Next lnqAggergatorsItem
    ...
    Private Sub FillTable(strFilterText As String)
        ...
        If intCurrentPageNo > 0 Then
            ...
            Dim lnqAggergators = OdbVAS.tbl_Aggregators.Where(Function(x) x.Name.IndexOf(strFilterText) <> -1 OrElse x.IMIPreNumber.IndexOf(strFilterText) <> -1 OrElse x.PardisPreNumber.IndexOf(strFilterText) <> -1).OrderByDescending(Function(x) x.ID).Skip(intSkip).Take(mdlGeneral.cnst_RowsCountInPage).ToList()
