Public Class VASServiceMembershipSerialManagement
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            If Request.QueryString("Save") IsNot Nothing AndAlso Request.QueryString("Save") = "OK" Then
                Bootstrap_Panel.ShowMessage("سرویس سریالی با موفقیت ذخیره شد", False)
            ElseIf Request.QueryString("Edit") IsNot Nothing AndAlso Request.QueryString("Edit") = "OK" Then
                Bootstrap_Panel.ShowMessage("سرویس سریالی با موفقیت ویرایش شد", False)
            ElseIf Request.QueryString("Save") IsNot Nothing AndAlso Request.QueryString("Save") = "NO" Then
                Bootstrap_Panel.ShowMessage("در فرآیند ذخیره سرویس سریالی خطا رخ داده است", True)
            ElseIf Request.QueryString("Edit") IsNot Nothing AndAlso Request.QueryString("Edit") = "NO" Then
                Bootstrap_Panel.ShowMessage("در فرآیند ویرایش سرویس سریالی خطا رخ داده است", True)
            End If
        ...
        Else
            ...
            If hdnAction.Value.StartsWith("E") = True Then
                ...
                Response.Redirect("VASServiceMembershipSerialEdit.aspx")
            End If
...
#Region "Bootstrap_Panel Events"
    ...
    Private Sub Bootstrap_Panel_Panel_New_Click(sender As Object, e As EventArgs) Handles Bootstrap_Panel.Panel_New_Click
        Response.Redirect("VASServiceMembershipSerialNew.aspx")
...
#Region "Table Functions"
    ...
    Private Function GetRecordCount() As Integer
        ...
        Dim intRecordCount = OdbVAS.tbl_VASServiceMembership.Where(Function(x) x.IsNewsContent = False).Count
    ...
    Private Sub FillTable()
        ...
        If intCurrentPageNo > 0 Then
            ...
            Dim OdbVAS As New BusinessObject.Context.dbVASEntities
            Dim lnqVASServiceMembership = OdbVAS.tbl_VASServiceMembership.Where(Function(x) x.IsNewsContent = False).OrderByDescending(Function(x) x.FK_VASServiceID).Skip(intSkip).Take(mdlGeneral.cnst_RowsCountInPage).ToList()

            intStartRecord = (intCurrentPageNo - 1) * mdlGeneral.cnst_RowsCountInPage + 1
            intEndRecord = intStartRecord + lnqVASServiceMembership.Count - 1

            Call BuildTable(intStartRecord, lnqVASServiceMembership)
    ...
    Private Sub BuildTable(intStartRecord As Integer, lnqVASServiceMembership As IEnumerable(Of BusinessObject.Context.tbl_VASServiceMembership))
        ...
        For Each lnqVASServiceMembershipItem In lnqVASServiceMembership
            ...
            tbCell.InnerHtml = "<input type = ""checkbox"" value="""" id=""chkk" & lnqVASServiceMembershipItem.FK_VASServiceID & """   />"
            tbRow.Cells.Add(tbCell)
            ...
            tbCell = New HtmlTableCell
            tbCell.InnerHtml = "<a href=""#"" onclick=""btnEdit_ClientClick(" & lnqVASServiceMembershipItem.FK_VASServiceID & ")"">&nbsp;" & lnqVASServiceMembershipItem.tbl_VASService.ServiceName & "<br />" & lnqVASServiceMembershipItem.tbl_VASService.tbl_CategoryVASService.Title & "&nbsp;</a>"
            tbCell.Attributes.Add("title", "برای ویرایش روی لینک، کلیک نمایید")
            tbRow.Cells.Add(tbCell)


            tbCell = New HtmlTableCell
            tbCell.InnerHtml = lnqVASServiceMembershipItem.tbl_VASService.tbl_Aggregators.Name & "<br />" & lnqVASServiceMembershipItem.tbl_VASService.theWholeNumber & " / " & If(lnqVASServiceMembershipItem.tbl_VASService.IsPardis = True, "پردیس", "IMI")
            tbRow.Cells.Add(tbCell)

            tbCell = New HtmlTableCell
            tbCell.InnerHtml = lnqVASServiceMembershipItem.tbl_VASService.tbl_User.Username & "<br />" & mdlGeneral.GetPersianDateTime(lnqVASServiceMembershipItem.tbl_VASService.STime)
            tbRow.Cells.Add(tbCell)


            tbCell = New HtmlTableCell
            tbCell.InnerHtml = lnqVASServiceMembershipItem.tbl_VASService.AggergatorServiceID & "<br />" & If(lnqVASServiceMembershipItem.tbl_VASService.IsActive = True, "فعال", "غیر فعال")
            tbRow.Cells.Add(tbCell)
